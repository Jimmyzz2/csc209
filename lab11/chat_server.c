...
int accept_connection(int fd, struct sockname *usernames) {
    ...
    usernames[user_index].username = malloc(sizeof(char)*40);
    read(client_fd, usernames[user_index].username, 40 * sizeof(char));
...
int read_from(int client_index, struct sockname *usernames) {
    ...
    strcat(buf, ": ");
    char buf2[BUF_SIZE + 1 - strlen(usernames[client_index].username) - 2];
    memset(buf2, '\0',BUF_SIZE + 1 - strlen(usernames[client_index].username) - 2 );
    int num_read = read(fd, buf2 , BUF_SIZE);
    strcat(buf, buf2);
