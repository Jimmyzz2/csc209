...
int compare_time(struct timespec* t1, struct timespec* t2) {
    // compare two struct timespec*, return -1, 0, 1 if t1 < t2, t1 == t2, t1 > t2
...
struct timespec* zero_timespec() {
    // helper function that creates struct timespec * with value 0
...
int action(struct rule_node* node) {
    // helper function that take action of a rule_node's args
...
struct timespec *recursive_dep(struct rule_node* node) {
    ...
    while (curr != NULL) {
        ...
        if (file_exist(curr->rule->target) == 0) {
            ...
            stat(fd, buf);
            new_time->tv_sec = buf->st_mtim.tv_sec;
            new_time->tv_nsec = buf->st_mtim.tv_nsec;
        }    
    ...
    if (file_exist(node->target) == 0) {
        ...
        struct stat *buf = malloc(sizeof(struct stat));
        stat(fd, buf);
        node_time->tv_sec = buf->st_mtim.tv_sec;
        node_time->tv_nsec = buf->st_mtim.tv_nsec;
    }
