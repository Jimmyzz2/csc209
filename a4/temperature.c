...
int main(int argc, char **argv) {
	...
	while (1) {
		...
		// first message has to be handshake
        if (msgno == 1) {
            cig.hdr.device_id = -1;
            cig.hdr.type = HANDSHAKE;
            msgno += 1;
            char * message1 = serialize_cignal(cig);
            int num_written = write(peerfd, message1, sizeof(char) * CIGLEN);
            if (num_written < 0) {
                perror("Temperature client: write");
                exit(1);
            }
            int num_read = read(peerfd, &message2, sizeof(char) * CIGLEN);
            if (num_read < 0) {
                perror("Temperature client: read");
                exit(1);
            }
            unpack_cignal(message2, &cig);
        } else {
            cig.hdr.type = UPDATE;
            char * message1 = serialize_cignal(cig);
            int num_written = write(peerfd, message1, sizeof(char) * CIGLEN);
